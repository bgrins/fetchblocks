...
dntShim.Deno.test("fetchblocks - builtins", async () => {
    ...
    assertEquals(await jsEval("return builtins.jmespath(input, options)", { a: 1 }, "a"), 1);
    assertEquals(await jsEval("return builtins.noop(input, options)", { a: 1 }), {
        a: 1,
    });
    assertEquals(await jsEval("return builtins.csv_to_json(input, options)", "foo,bar,baz", {}), {
        data: [["foo", "bar", "baz"]],
        errors: [],
        meta: {
            aborted: false,
    ...
    assertEquals(await jsEval("return builtins.json_to_csv(input, options)", [["foo", "bar", "baz"]], {}), "foo,bar,baz");
    ...
    assertObjectMatch(await jsEval("return builtins.md_to_json(input, options)", "# header"), [
        {
            depth: 1,
            raw: "# header",
            text: "header",
...
dntShim.Deno.test("fetchblocks - jmespath", async () => {
    assertEquals(await fetchblocks.run(JSON.parse(`[
      {
        "resource":
...
dntShim.Deno.test("loadFromText with inheritance", async () => {
    ...
    let block = await fetchblocks.loadFromText(dntShim.Deno.readTextFileSync("./testdata/blocks/multiple-json.json"), null, {
        id: "n_top_stars",
    });
    let ret = await block.run({
        dataset: {
...
dntShim.Deno.test("likely blocks", async () => {
    assertEquals(fetchblocks.getLikelyBlocksFromText(`{
      "fetch_1": [{ "resource": "example.com" }],
      "fetch_2": [{ "resource": "example.org" }]
    }`), ["fetch_1", "fetch_2"]);
    assertEquals(fetchblocks.getLikelyBlocksFromText(`[{ "resource": "example.com" }]`), ["default"]);
    ...
    `), ["fetch_1", "fetch_2"]);
    assertEquals(fetchblocks.getLikelyBlocksFromText(`<fetch-block resource="http://example.com"></fetch-block>`), ["default"]);
