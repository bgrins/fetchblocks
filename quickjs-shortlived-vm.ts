...
export async function makeShortlivedVm(timeout: number = -1) {
  ...
  //     InterruptAfterDeadline(Date.now() + timeout)
  //   );
  // }
  ...
  const newNumber: typeof vm.newNumber = (num: any) => {
    const handle = vm.newNumber(num);
    disposables.add(handle);
    return handle;
  };

  const newString: typeof vm.newString = (str: any) => {
  ...
  return {
    ...
    //  */
    // removeInterruptHandler: typedBind(
